...
class ConcSdouble(Sdouble, ConcSnumber):
    ...
    def __new__(cls, value):
        # Intern the three named singletons (ZERO/ONE/MINUS_ONE).  Floating
        # point identity is otherwise not useful (and would be unsound for
        # NaN, since ``NaN == NaN`` is False).
        if cls is ConcSdouble and isinstance(value, (int, float, bool)):
            try:
                v = float(value)
            except (TypeError, ValueError):
                return object.__new__(cls)
            z = getattr(ConcSdouble, "ZERO", None)
            if z is not None and v == 0.0:
                return z
            o = getattr(ConcSdouble, "ONE", None)
            if o is not None and v == 1.0:
                return o
            mo = getattr(ConcSdouble, "MINUS_ONE", None)
    ...
    def __init__(self, value: float) -> None:
        v = float(value)
        if getattr(self, "_value", _UNSET_DOUBLE) == v:
            return

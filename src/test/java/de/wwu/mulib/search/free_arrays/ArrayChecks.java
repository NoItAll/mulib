...
public class ArrayChecks {
    ...
    public static Sbool checkConcreteSelect0() {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (objs.length().notEqChoice(se.concSint(2), se)) {
                throw Mulib.fail();
            }
    ...
    public void checkConcreteIllegalAccessWithOOB() {
        TestUtility.getAllSolutions((mb) -> {
            ...
            assertEquals(1, result.size());
            assertTrue(result.stream().allMatch(ps -> ps instanceof ThrowablePathSolution));
    ...
    public void checkSymArraySelect() {
        TestUtility.getAllSolutions((mb) -> {
            ...
            assertEquals(4, result.size());
            assertEquals(1, result.stream().filter(ps -> !(ps instanceof ThrowablePathSolution)).count());
            assertEquals(3, result.stream().filter(ps -> ps instanceof ThrowablePathSolution).count());
    ...
    public static Sbool checkSymStore0() {
        SymbolicExecution se = SymbolicExecution.get();
        Sarray.SintSarray objs = se.sintSarray(se.symSint(), true);
        if (objs.length().notEqChoice(se.concSint(1), se)) {
            throw Mulib.fail();
        }
    ...
    public void checkMultipleArrays() {
        TestUtility.getAllSolutions((mb) -> {
            ...
            assertEquals(7, result.stream().filter(ps -> !(ps instanceof ThrowablePathSolution)).count());
            assertTrue(result.stream().anyMatch(ps -> ps instanceof ThrowablePathSolution));
        }, "checkMultipleArrays");
    }
...
//    @Test @Disabled(value="Subsumed by FreeArraysExec.testSimpleSort1")
    ...
    public static Sarray.SintSarray simpleSort() {
        ...
        if (se.notEqChoice(a.length(), se.concSint(b.length))) {
            throw Mulib.fail();
        }
    ...
    public void testSarraySarraysWithNulls() {
        TestUtility.getAllSolutions(
                mb -> {
                    ...
                    assertEquals(2, solutions.size());
                    assertTrue(solutions.stream().anyMatch(s -> s instanceof ThrowablePathSolution && s.getSolution().returnValue instanceof NullPointerException));
                    assertTrue(solutions.stream().anyMatch(s -> !(s instanceof ThrowablePathSolution) && ((Integer) s.getSolution().returnValue) == 2));
                },
                "sarraySarraysWithNulls"
        );
    }
